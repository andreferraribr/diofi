...
```{r importar saldo dos repasses a programar}
...
```{r importa etapas da execução orçamentária}
...
chave_pago <- chave_pago %>%  mutate (chave_ugr_fonte= paste0(ugr_cod,fonte_det_cod))
```
...
tabela(etapas %>% group_by( uo_cod, ugr, fonte_det_cod, pi, nd_cod, item) %>% summarise(saldo = sum(saldo)) %>%  pivot_wider( names_from = item, values_from = saldo))
...
rep_lim <- full_join(repasse %>% group_by(fonte_det_cod) %>% summarise(repasse = ( sum(saldo))), limite %>% group_by(fonte_det_cod) %>% summarise(limite = ( sum(saldo))))

repasse %>% group_by(fonte_det_cod) %>% summarise(q = unique(ug_cliente)) %>% count(fonte_det_cod)


necessidade <- etapas %>% filter(item_cod %in% c("19","30", "32")) %>% group_by( fonte_det_cod, item) %>% summarise(saldo = sum(saldo)) %>%  pivot_wider( names_from = item, values_from = saldo)

necessidade_financeira <- full_join(necessidade, rep_lim)

necessidade_financeira <- necessidade_financeira %>% mutate(saldo_imeditado = limite - `DESPESAS LIQUIDADAS A PAGAR(CONTROLE EMPENHO)`,
                                                            saldo_total = limite + repasse - `CREDITO DISPONIVEL`- `DESPESAS EMPENHADAS A LIQUIDAR (CONTROLE EMP)`- `DESPESAS LIQUIDADAS A PAGAR(CONTROLE EMPENHO)`)

tabela(necessidade_financeira)
...
tabela (nes_fin_imedita)

necessidade_financeira <- necessidade_financeira %>%  mutate(necessidade = `CREDITO DISPONIVEL`+ `DESPESAS EMPENHADAS A LIQUIDAR (CONTROLE EMP)`+ `DESPESAS LIQUIDADAS A PAGAR(CONTROLE EMPENHO)`)
