...
renderDT({
tabela(custos %>% filter (imovel_custo == input$imovel) %>%  group_by(fav, mes_nome) %>% summarise(custo = sum(custo)) %>% pivot_wider( names_from = mes_nome, values_from = custo) %>% mutate_all(~replace(., is.na(.), 0))%>%
  adorn_totals("col"),"Total"
) 
})
...
tabela(etapas %>% group_by(  pi, cc_chave,   item) %>% summarise(saldo = sum(saldo)) %>%  pivot_wider( names_from = item, values_from = saldo))
