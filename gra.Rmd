...
realizado <- custos %>% 
  ...
  group_by(favorecido, favorecido_cod, mm_ano_cod, mes, mes_nome, chave) %>% 
 ...
 historico<- ggplot( df_br, aes(  x= mes , y=cumsum(custo))) +
  ...
  geom_hline(yintercept=input$limite, colour = "red", linetype = "dashed") +
   ...
   # https://stackoverflow.com/questions/43099990/r-lubridate-weekdays-in-local-language
    annotate("text", label = paste0 ("custos pagos até ", month(max(realizado$mes), label = TRUE, abbr = FALSE, 
...
tabela(custos %>% filter (org == "PFN", startsWith (pi_cod, "RAT")) %>%  group_by( favorecido,ndd_cod, mes_nome, ne, doc_habil) %>% summarise(custo = sum(custo)) ) 
```
...
```{r}
tabela(repasse %>% group_by( cc, fonte_deta) %>% summarise(saldo = sum(saldo)))
```
Row
--------------------------------------------------------------------------
...
```{r}
tabela(limite %>% group_by( cc, fonte) %>% summarise(saldo = sum(saldo)))
```
