...
```{r baixar arquivo do google drive, eval=FALSE, include=FALSE}
...
fontes_inicial <- read_excel ("~/diofi/saldo_inicial_fontes.xlsx")
...
credito <- dados("~/Dropbox/r_credito_2_2021.xlsx", "~/diofi/tg2r_credito_2.xlsx")

limite <- dados("~/Dropbox/r_limite_2021.xlsx", "~/diofi/tg2r_financeiro.xlsx")

repasse <- dados("~/Dropbox/r_repasse_2021.xlsx", "~/diofi/tg2r_financeiro.xlsx")

etapas <- dados("~/Dropbox/r_etapas_2021.xlsx", "~/diofi/tg2r_etapas.xlsx")

# etapas_datas <- dados("~/Dropbox/r_etapas_datas_2021.xlsx", "~/Dropbox/tg2r_etapas_datas.xlsx")
# 
# #  importar relatório gerado no Tesouro Gerencial com dados sobre o custos e renomear as colunas
custos <- dados("~/Dropbox/r_custos_2021.xlsx", "~/diofi/tg2r_custos.xlsx")
...
```{r}
pf_recebido <- repasse %>% group_by( fonte_cod, ug_cliente) %>% filter(fonte_cod %in% c("00","44"), doc_tipo_cod == "PF") %>% summarise(recebido = sum(-saldo))%>% 
  #  alterar NA por 0
  mutate_all(~replace(., is.na(.), 0))

pf_recebido <- pf_recebido %>%  mutate(org = case_when(
  ug_cliente == "410003COORDENACAO-GERAL DE RECURSOS LOGISTICOS" ~ "MCTIC",
  ug_cliente == "110120AGENCIA BRASILEIRA DE INTELIGENCIA" ~ "ABIN",
  ug_cliente == "400081SUPERINTENDENCIA REGIONAL DO TRABALHO/SC" ~ "TRABALHO",
  ug_cliente == "370001DIRETORIA DE GESTAO INTERNA(CGU)"  ~ "CGU",
  ug_cliente == "170006COORDENACAO-GERAL DE GESTAO DE PESSOAS - MF"  ~ "PESSOAL",
  ug_cliente == "170013SETORIAL ORCAMENTARIA E FINANCEIRA / ME"  ~ "GRA",
  TRUE  ~ "outras orgs"))

pf_recebido <- pf_recebido %>%  select(org,fonte_cod, recebido)

pago <- etapas %>% group_by( fonte_cod, uo, ugr) %>% filter(fonte_cod %in% c("00","44"), item_cod == "56") %>% summarise(pago= sum(saldo))%>% 
  #  alterar NA por 0
  mutate_all(~replace(., is.na(.), 0))

pago <- pago %>%  mutate(org = case_when(
  uo == "410003COORDENACAO-GERAL DE RECURSOS LOGISTICOS" ~ "MCTIC",
  uo == "AGENCIA BRASILEIRA DE INTELIGENCIA - ABIN" ~ "ABIN",
  ugr == "400081SUPERINTENDENCIA REGIONAL DO TRABALHO/SC" ~ "TRABALHO",
  uo == "CONTROLADORIA GERAL DA UNIAO"  ~ "CGU",
  TRUE  ~ "outras orgs"))

pago <- pago  %>%  select(org,fonte_cod, pago)

fonte_final <- left_join(fontes_inicial, pf_recebido, by = c("org" = "org", "fonte_cod"= "fonte_cod" ))%>% 
  #  alterar NA por 0
  mutate_all(~replace(., is.na(.), 0))

fonte_final <- left_join(fonte_final, pago, by = c("org" = "org", "fonte_cod"= "fonte_cod" ))%>% 
  #  alterar NA por 0
  mutate_all(~replace(., is.na(.), 0))

fonte_final <- fonte_final %>% mutate(saldo_final = `saldo inicial`+recebido-pago) %>% select(-uo)

(limite %>% filter(fonte_cod == "00") %>% summarise(saldo = sum(saldo)))-(fonte_final %>% filter(fonte_cod== "00") %>% summarise(saldo_final=sum(saldo_final)))

(limite %>% filter(fonte_cod == "44") %>% summarise(saldo = sum(saldo)))-(fonte_final %>% filter(fonte_cod== "44") %>% summarise(saldo_final=sum(saldo_final)))
